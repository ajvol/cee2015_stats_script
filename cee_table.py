...
def PublishStats():
    max_line = 80.0
    t = u''
    t = t + u'{{Notice|This is bot generated statistics about new articles from [[Wikimedia CEE Spring 2017/Article Lists|proposal lists]]. It\'s updated every 2 hours. ' \
            u'Other statistics based on contest template approach, you can find at [[User:BaseBot/CEES/MMXVII/Wikis]].}}' + '\n'
    ...
    total2016 = 5920
    max_val = max([total, total2016])
    point = 1.0 * max_line / max_val
    rep = int(round(point * total2016))
    t += u'|-\n'
    t += u'| 2016 CEE Spring articles - || ' + str(total2016) + ' || ' + (u'▒' * rep) + '\n'
    rep = int(round(point * total))
    t += u'|-\n'
    t += u'| 2017 CEE Spring articles - || ' + str(total) + ' || ' + (u'▒' * rep) + '\n'
    t += u'|}\n'
    ...
    for co in sorted(stats_by_country.keys(), lambda x, y: stats_by_country[y] - stats_by_country[x]):
        ...
        if debug:
            count_str = ' [[User:Botik/Stats/'+co+'|'+str(stats_by_country[co])+']]'
        else:
            count_str = ' [[Wikimedia_CEE_Spring_2017/Structure/Statistics/'+co+'|'+str(stats_by_country[co])+']]'
    ...
    if not debug:
        savePage('Wikimedia_CEE_Spring_2017/Structure/Statistics', t)
    else:
...
def get_subpage_ids(country):
    # https://meta.wikimedia.org/wiki/Wikimedia_CEE_Spring_2017/Structure/Bulgaria
    # https://meta.wikimedia.org/wiki/Wikimedia_CEE_Spring_2017/Structure/Bulgaria/Science

    subpage_list=[]
    req = urllib2.Request(
        'https://meta.wikimedia.org/w/api.php?format=json&action=query&list=allpages&aplimit=max&apprefix=Wikimedia+CEE+Spring+2017%2FStructure%2F' + urllib2.quote(
...
def get_county_list():
    ...
    req = urllib2.Request(
        ...
        urllib2.quote('Template:Wikimedia CEE Spring 2017 Article Lists by Country'.encode('utf-8')) + '&continue=')
    resp = urllib2.build_opener().open(req).read()
    r_json = json.loads(resp)
    ...
    mat = ''
    for line in wiki_text.split('\n'):
        # File:Flag_of_Albania.svg|<br />[[Wikimedia_CEE_Spring_2017/Structure/Albania|{{Label|Q222}}]]|link=[[Wikimedia_CEE_Spring_2017/Structure/Albania]]|alt=Albania
        match = re.search(ur'\|Wikimedia.CEE.Spring.2017/Structure/([^\|]*?)\|', line, re.IGNORECASE)
        if match is not None:
            mat = match.group(1).strip()
...
def get_lang_list():
    ...
    q='Q28361633' # Wikimedia CEE Spring 2017
...
'''
    ...
    if not debug:
        savePage('Wikimedia_CEE_Spring_2017/Structure/Statistics/'+country, txt)
    else:
...
lang_names = {'az': u'Azerbaijani - azərbaycan dili',
              'ba': u'Bashkir - башҡорт теле',
              'be': u'Belarusian - беларуская мова',
              'be-tarask': u'Belarusian (Taraškievica) - беларуская мова (тарашкевіца)',
              'bg': u'Bulgarian - български език',
              'bs': u'Bosnian - bosanski jezik',
              'ce': u'Chechen - нохчийн мотт',
              'crh': u'Crimean Tatar - Къырымтатар',
              'cs': u'Czech - čeština',
              'cv': u'Chuvash - чӑваш чӗлхи',
              'de': u'German - Deutsche',
              'el': u'Greek - ελληνικά',
              'eo': u'Esperanto',
              'et': u'Estonian - eesti',
              'hr': u'Croatian - hrvatski jezik',
              'hu': u'Hungarian - magyar',
              'hy': u'Armenian - Հայերեն',
              'ka': u'Georgian - ქართული',
              'kk': u'Kazakh - қазақ тілі',
              'lt': u'Lithuanian - lietuvių kalba',
              'lv': u'Latvian - latviešu valoda',
              'mk': u'Macedonian - македонски јазик',
              'myv': u'Erzya - эрзянь кель',
              'os': u'Ossetian - ирон æвзаг',
              'pl': u'Polish - polszczyzna',
              'ro': u'Romanian - limba română',
              'ru': u'Russian - русский язык',
              'rue': u'Rusyn - русин',
              'sah': u'Sakha - саха тыла',
              'sh': u'Serbo-Croatian - srpskohrvatski jezik',
              'sk': u'Slovak - slovenčina',
              'sl': u'Slovene - slovenščina',
              'sq': u'Albanian - Shqip',
              'sr': u'Serbian - српски језик',
              'tr': u'Turkish - Türkçe',
              'tt': u'Tatar - татар теле',
              'uk': u'Ukrainian - українська мова'}
        ...
        try:
            ...
            for key in sorted(item_link_dict):
                if (key != 'en') and (len(item_link_dict[key]) > 1):
                    ...
                    # new articles
                    if (first_edit >= datetime.strptime('2017-03-21', "%Y-%m-%d")) and (first_edit < datetime.strptime('2017-06-01', "%Y-%m-%d")):
                    ...
                    # old articles
                    elif (first_edit < datetime.strptime('2017-03-21', "%Y-%m-%d")) and full_processing:
                        ...
                        if (last_edit >= datetime.strptime('2017-03-21', "%Y-%m-%d")) and (last_edit < datetime.strptime('2017-06-01', "%Y-%m-%d")):
                        ...
                        elif last_edit < datetime.strptime('2017-03-21', "%Y-%m-%d"):
