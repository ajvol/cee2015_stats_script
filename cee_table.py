...
def getFirstEdit(server, title):
    ...
    if server + ':' + title in cache:
        ...
        co_list_req = urllib2.Request("http://" + server.replace('be-tarask',
                                                                 'be-x-old') + ".wikipedia.org/w/api.php?format=json&action=query&prop=revisions&titles=" + urllib2.quote(
...
def ResolveRedirects(server, title):
    ...
    try:
        ...
        co_list_req = urllib2.Request("http://" + server.replace('be-tarask',
                                                                 'be-x-old') + ".wikipedia.org/w/api.php?format=json&action=query&titles=" + urllib2.quote(
